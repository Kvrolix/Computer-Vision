...
%}
    ...
    % Check if there is a red car in the image
    percentageRed = sum(binaryImage(:)) / numel(binaryImage) * 100;
    if percentageRed > 1
        disp('There is a red car in the image!');
        ...
        if ~isempty(stats)
            ...
            % Check if the ratio is close to 1:3
            bound_box_width = boundingBox(3);
            bound_box_height = boundingBox(4);
            ratio = bound_box_width / bound_box_height;
            if abs(ratio - 1/3) < 0.1
                disp('Detected a car with a ratio of 1:3');
            else
                disp('The car does not have a ratio of 1:3');
            end
        else
            disp('No regions found.');
        end
